...
int costCalculate(int mat[N][N], int final[N][N]) {
    int count = 0;
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++)
...
int calculateHeu(int mat[N][N], int goal[N][N]) {
    int heu = 0;
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++)
            if (mat[i][j] != goal[i][j])
...
struct comp {
    bool operator()(const Node *lhs, const Node *rhs) const {
        return lhs->heu > rhs->heu;  // Order by heu value for Greedy Search
    }
};
...
void solve(int initial[N][N], int x, int y, int goal[N][N]) {
    ...
    root->heu = calculateHeu(initial, goal);  // Calculate initial heuristic
    pq.push(root);
    ...
    while (!pq.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            if (checkLimit(min->x + row[i], min->y + col[i])) {
                ...
                child->cost = costCalculate(child->mat, goal);
                child->heu = calculateHeu(child->mat, goal);
                pq.push(child);
